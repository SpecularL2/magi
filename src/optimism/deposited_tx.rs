...
impl AttributesDeposited {
    fn from_block_info(l1_info: &L1Info, seq: u64, batch_timestamp: u64, config: &Config) -> Self {
        ...
        let optimism_system_config = l1_info.system_config.as_optimism().unwrap();
        ...
        Self {
            ...
            sequence_number: seq,
            batcher_hash: optimism_system_config.batcher_hash(),
            fee_overhead: optimism_system_config.l1_fee_overhead,
            fee_scalar: optimism_system_config.l1_fee_scalar,
            gas,
            is_system_tx,
        }
    }
...
pub struct OptimismTransactionDeriver;

impl TransactionDeriver for OptimismTransactionDeriver {
    fn derive_transactions(
        &self,
        config: &Config,
        state: &RwLock<State>,
        ...
        let attributes_tx =
            self.derive_attributes_deposited(config, sequence_number, l1_info, batch.timestamp);
...
impl OptimismTransactionDeriver {
    pub fn new() -> Self {
        Self {}
    }
    ...
    fn derive_attributes_deposited(
        ...
        let attributes_deposited = AttributesDeposited::from_block_info(
            l1_info,
            sequence_number,
            batch_timestamp,
            config,
    ...
    fn derive_user_deposited(
        ...
        state
            ...
            .map(|info| &info.extra.as_optimism().unwrap().user_deposits)
            .map(|deposits| {
                deposits
                    .iter()
                    .map(|deposit| {
...
impl Default for OptimismTransactionDeriver {
    fn default() -> Self {
        Self::new()
    }
}